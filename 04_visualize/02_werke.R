...
werke_server <- function(id) {
    moduleServer(id, function(input, output, session) {
        ...
        output$werke_table <- renderDT({
            df <- daten_gefiltert() %>%
                dplyr::select(
                    vorgangsfolge, Anzahl, Median_LT, Durchschnitt_Abweichung, 
                    Durchschnitt_Liefermenge, Servicelevel_prozent
                )
            datatable(
                df,
...
werke_ui <- function(id) {
    ...
    tagList(
        ...
        fluidRow(
            box(
                title = "Ø Liefermenge je Vorgangsfolge",
                width = 12, solidHeader = TRUE,
