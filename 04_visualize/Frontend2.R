...
einstieg_kategorie <- function(df, kategorie_spalte) {
    df %>%
        ...
        summarise(
            ...
            anzahl_auftraege = n(),
            differenz_lt = round(mean(abweichung, na.rm = TRUE), 1),
            platzhalter = "",
            .groups = "drop"
        ) %>%
    ...
    output$main_ui <- renderUI({
        if (nav$seite == "start") {
            ...
            fluidPage(
                ...
                tags$div(
                    tags$strong("Anzahl Aufträge"), ": Anzahl aller Aufträge in dieser Ausprägung", tags$br(),
                    tags$strong("Average LT"), ": ", tags$em("Text: Formel muss noch formuliert werden"), tags$br(),
                    tags$strong("Differenz LT"), ": ", tags$em("Text: Formel muss noch formuliert werden"), tags$br(),
